...
void redraw(double freq1, double freq2, double freq3) 
{
	...
	{
		double s = std::sin(2 * M_PI * freq1 * scale * i / samplerate) + 
				   std::sin(2 * M_PI * freq2 * scale * i / samplerate) + 
				   std::sin(2 * M_PI * freq3 * scale * i / samplerate);

		SDL_RenderDrawPoint(renderer, (int) (600.0 - (double)(i)*dx), s * 100 / 3.0 + 150);
	}
...
int WinMain(int argc, char** argv) {
	...
	std::cout << "Frequenza 1: ";
	std::cin >> freq1;
	std::cout << "Frequenza 2: ";
	std::cin >> freq2;
	std::cout << "Frequenza 3: ";
	std::cin >> freq3;
	...
	{
		// y = sin(2 * pi * f * t)
		double s = std::sin(2 * M_PI * freq1 * i / samplerate) +
				   std::sin(2 * M_PI * freq2 * i / samplerate) + 
				   std::sin(2 * M_PI * freq3 * i / samplerate);

		// write the raw audio data to the output file
		// multiply the value by the maximum volume
		write(of, (short) (s * 32767.0 / 3.0), 2);
